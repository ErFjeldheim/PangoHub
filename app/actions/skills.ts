...
import type {
  Skill as PBSkill,
  ProfileSkill as PBProfileSkill,
...
export async function findSkillByName(
  ...
  try {
    const record = await pb
      .collection("skills")
      .getFirstListItem<PBSkill>(`name~"${trimmed}"`);
    return { id: record.id, name: record.name };
  } catch {
    return null;
  }
...
export async function removeProfileSkill(params: {
  ...
  try {
    const record = await pb
      .collection("profile_skills")
      .getFirstListItem(
        `user="${params.profileId}" && skill="${params.skillId}"`,
      );
